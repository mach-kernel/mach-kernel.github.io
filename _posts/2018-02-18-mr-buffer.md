...
This is how I built [mrbuffer](https://github.com/mach-kernel/mrbuffer).
...
The observant reader will exclaim that the 65816 supports 24 bit addressing, so specifying the bank is not a problem. This is correct, however we need to learn more about the ROM to understand why exactly this won't work. To retain compatibility with existing Apple II software, the original Apple II ROM must ship with the IIgs such that software written for the Apple II can still run. That is, if original Apple II programs use the COUT routine, they will also need to use it on the IIgs regardless of the 6502 emulation mode. Therefore, these ROM functions are written in 6502 assembly as opposed to native 65816 assembly, which poses two problems. Suppose we invoke a `jsl $00FDED`, with the 24-bit "long" address of the `COUT` routine. The computer _will arrive_ at the correct instructions -- and given the instruction compatibility and overlap with the 65816 -- it will run them. In native mode, however, our registers are all _twice as large_, so the code will perform in an unexpected manner and cause the GS to crash to monitor (the ROM contains a small assembler/monitor that is invoked during a crash). Then, even if the computer were to execute these instructions successfully (and display the char), the `jsr` used to exit the routine will jump to a 16-bit address, so if bank boundaries were crossed, you will _never_ get back to your code. We would need a native IIgs ROM `COUT` (which, I do not believe exists for the 40-char video page).
...
The observant reader will exclaim that we're all out of available registers. Correct! But if we need to shuffle values, we can still use the stack to do so with instructions such as `pha` to push `A` to stack, and `pla` to pull `A` from stack. You'll see an example of this when we build the row and character counting mechanisms. 
...
Remember when we spoke about `OMF` being _relocatable_? With `phk` we push the program bank register (the CPU very obviously knows where it is reading the program from, because it is executing it!) onto the stack and then pull it right back off with `plb`, but into the data bank register. Although we do not have any strings or constants that we want to place in (i.e. using `db` "define byte" instructions we can make strings) our editor, if we do, it will be really confusing if we make reads to some offset relative to our code and get back insane values by virtue of reading from another bank (also, ask me how I know)!
...
And of course, here's an ASCII table to make life easier. I embarassingly relied more on the `brk` method above to identify characters because I was lazy:
